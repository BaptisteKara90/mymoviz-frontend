...
function Home() {
  ...
  useEffect(()=>{
    fetch('https://mymoviz-backend-mauve.vercel.app/movies')
    .then(response => response.json())
    .then(data => {
      setFetchMovies(data.movies)
    })
  },[])
  ...
  const movies = fetchMovies.map((data, i) => {
    const isLiked = likedMovies.some(movie => movie === data.title);
    return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.original_title} overview={data.overview} poster={data.poster_path} voteAverage={data.vote_average} voteCount={data.vote_count} />;
  });
